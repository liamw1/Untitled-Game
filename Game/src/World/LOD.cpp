...
namespace LOD
{
  ...
  static Array2D<Terrain::CompoundSurfaceData, c_NumCells + 1> generateNoise(Octree::Node* node)
  {
    EN_PROFILE_FUNCTION();
  ...
  static bool needsMesh(Octree::Node* node, const Array2D<Terrain::CompoundSurfaceData, c_NumCells + 1>& noiseValues)
  {
  ...
  static Array2D<Vec3, c_NumCells + 1> calcNoiseNormals(Octree::Node* node, const Array2D<Terrain::CompoundSurfaceData, c_NumCells + 1>& noiseValues)
  {
    length_t cellLength = node->length() / c_NumCells;
    Vec2 LODAnchorXY = Chunk::Length() * static_cast<Vec2>(node->anchor);
  ...
  static NoiseData interpolateNoiseData(Octree::Node* node, const Array2D<Terrain::CompoundSurfaceData, c_NumCells + 1>& noiseValues, const Array2D<Vec3, c_NumCells + 1>& noiseNormals, const BlockIndex& cornerA, const BlockIndex& cornerB, float s)
  {
    length_t LODFloor = node->anchor.k * Chunk::Length();
    length_t cellLength = node->length() / c_NumCells;
  ...
  static void generatePrimaryMesh(Octree::Node* node, const Array2D<Terrain::CompoundSurfaceData, c_NumCells + 1>& noiseValues, const Array2D<Vec3, c_NumCells + 1>& noiseNormals)
  {
    EN_PROFILE_FUNCTION();
  ...
  static void generateTransitionMeshes(Octree::Node* node, const Array2D<Terrain::CompoundSurfaceData, c_NumCells + 1>& noiseValues, const Array2D<Vec3, c_NumCells + 1>& noiseNormals)
  {
    EN_PROFILE_FUNCTION();
