...
namespace Temp
{
  ...
  */
    ...
    */
    [[nodiscard]] std::vector<Chunk>::iterator loadChunk(const GlobalIndex& chunkIndex);
    ...
    /*
      ...
      \returns A valid iterator at the location of the chunk in m_BoundaryChunks.
    */
    [[nodiscard]] std::vector<Chunk>::iterator unloadChunk(Chunk& chunk);
    ...
    */
    [[nodiscard]] std::vector<Chunk>::iterator moveToGroup(std::vector<Chunk>::iterator iteratorPosition, ChunkType source, ChunkType destination);

    // NOTE: These methods are possible undefined behaviour, works for now
