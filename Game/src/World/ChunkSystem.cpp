...
{
  template<typename T>
  static bool isIn(const std::vector<T>& arr, const T* element)   // NOTE: This method is possible undefined behaviour, works for now
  {
#if GM_EXPERIMENTAL
  ...
  bool ChunkManager::loadNewChunks(int maxNewChunks)
  {
    ...
    if (m_ChunksLoaded == 0)
    {
      auto it = loadChunk(Player::OriginIndex());
      EN_ASSERT(isIn(m_BoundaryChunks, *it), "Chunk was not added to boundary chunks!");
    }
  ...
  std::vector<Chunk>::iterator ChunkManager::unloadChunk(Chunk& chunk)
  {
    ...
    for (Block::Face face : Block::FaceIterator())
    {
      ...
      if (chunkNeighbor)
      {
        ...
        auto it = moveToGroup(getIterator(chunkNeighbor, source), source, ChunkType::Boundary);
      }
    }
  ...
  static bool blockHasTransparency(BlockIndex blockIndex, const StackArray3D<const Chunk*, 3>& grid)
  {
    ...
    const Chunk* chunk = grid[chunkOffset.i + 1][chunkOffset.j + 1][chunkOffset.k + 1];
    if (chunk == nullptr)
      return false;
  ...
  bool ChunkManager::isOnBoundary(const Chunk& chunk) const
  {
    ...
    for (Block::Face face : Block::FaceIterator())
      if (!chunk.isFaceOpaque(face))
      {
        ...
        Block::Face vDir = static_cast<Block::Face>(v);
        Block::Face wDir = static_cast<Block::Face>(w);

        GlobalIndex adjacentChunkIndex = chunk.getGlobalIndex() + GlobalIndex::OutwardNormal(face);
        const Chunk* adjacentChunk = find(adjacentChunkIndex);

        if (adjacentChunk)
        {
          if ((!isLoaded(adjacentChunkIndex + GlobalIndex::OutwardNormal(vDir)) && !adjacentChunk->isFaceOpaque(vDir))    ||
              (!isLoaded(adjacentChunkIndex + GlobalIndex::OutwardNormal(!vDir)) && !adjacentChunk->isFaceOpaque(!vDir))  ||
              (!isLoaded(adjacentChunkIndex + GlobalIndex::OutwardNormal(wDir)) && !adjacentChunk->isFaceOpaque(wDir))    ||
              (!isLoaded(adjacentChunkIndex + GlobalIndex::OutwardNormal(!wDir)) && !adjacentChunk->isFaceOpaque(!wDir)))
            return true;
        }
      }
